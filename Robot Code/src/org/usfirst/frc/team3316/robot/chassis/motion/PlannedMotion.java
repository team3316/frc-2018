...
public class PlannedMotion {
	/**
	 * An object representing a specific time in the motion of the robot, with its
	 * position, velocity and acceleration during a motion in 1D.
	 * 
	 * @author D-Bug
	 */
	...
	public PlannedMotion(double maxAccel, double maxDecel, double vMax, double accelTime, double cruiseTime,
			double decelTime) {
		...
		/*
		 * Calculating some movement constants derived from specified ones for improved
		 * code clearance
		 */
		accelDistance = (maxAccel / 2) * Math.pow(accelTime, 2);
		cruiseDistance = vMax * (cruiseTime);
		decelDistance = vMax * decelTime + (maxDecel / 2) * Math.pow(decelTime, 2);
	}
	...
	public double getPosition(double time) {
		if (time <= accelTime) {
			...
			return accelDistance + cruiseDistance + vMax * timeInDecel + (maxDecel / 2) * Math.pow(timeInDecel, 2);
		} else {
	 ...
	 * since start of the movement, total distance traveled, current velocity and
	...
	public Step getStep(double time) {
		return new Step(getAcceleration(time), getVelocity(time), getPosition(time), time);
	}
