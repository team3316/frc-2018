...
public class VisionServer implements Runnable {
	...
	for (String var : vars) {
	    ...
	    String valueString = parts[1].substring(parts[1].indexOf('\'') + 1, parts[1].lastIndexOf('\''));
	...
	while (true) {
	    ...
	    // logger.finest("Time difference from last while: " +
		...
		// logger.finest("Packet data length: " +
